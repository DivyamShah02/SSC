...
min_lat, max_lat, min_lon, max_lon = get_bounds(lat=lat, lon=lng, radius_km=km)

step_size = 0.0001  # Step size in degrees
...
for i in range(int(len(coordinates)/n)):
    j = i * n
    j = random_number = random.randint(j-n, j)
