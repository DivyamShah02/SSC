...
def generate_feedback_pdf(data, client_name, svg_path=None):
    # Custom Canvas to set background color
    class BackgroundColorCanvas(canvas.Canvas):
        def __init__(self, *args, **kwargs):
            super().__init__(*args, **kwargs)
    ...
    # Title
    styles = getSampleStyleSheet()
    title = Paragraph(f"Feedback Score of {client_name}", styles['Title'])
    elements.append(title)
    ...
    # Table data: add column headers
    table_data = [["Sr. No.", "Property Name", "Feedback Score"]]
    ...
    for row in data:
        table_data.append([
            row.get("Sr. No."),
            row.get("property_name"),
            row.get("average_feedback"),
        ])

    # Create a table
    table = Table(table_data, colWidths=[40, 100, 120])  # Adjust column widths
